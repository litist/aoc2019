...
# appropriate size to fit in our grid
g_size = 16000
grid = np.zeros((g_size, g_size), dtype=np.uint16)
startpos = np.array([int(g_size/2), int(g_size/2)])
...
for w in content[0].split(','):
    ...
    for i in range(int(w[1:])):
        ...
        if grid[p0[0], p0[1]] == 0:
            # this was not required, but it makes it saver
            if steps0 > iu16.max:
              # print("MAx excced: ", grid[p0[0], p0[1]], steps0, iu16.max)
              grid[p0[0], p0[1]] = iu16.max
            else:
              grid[p0[0], p0[1]] = steps0


    if min(p0) < 0 or max(p0) > g_size:
        print("Out of bounds ", p0)
        exit(-1)

# run second wire and check for conflicts
p1 = np.copy(startpos)
minDist = 100000000
steps1 = 0
# read in first wire
for w in content[1].split(','):
    ...
    for i in range(int(w[1:])):
        ...
        if grid[p1[0], p1[1]] > 0:
            mDist = grid[p1[0], p1[1]] + steps1
            # print("Crossing at ", p1, grid[p1[0], p1[1]], mDist)
            if mDist < minDist:
                minDist = mDist

    # here we do not care about neagtive indices
    # if min(p1) < 0 or max(p1) > g_size:
    #     print("Out of bounds ", p1)
    #     exit(-1)

print("Solultion 2: ", minDist)




g_size = 16000
grid = np.zeros((g_size, g_size), dtype=np.int8)
startpos = np.array([int(g_size/2), int(g_size/2)])
